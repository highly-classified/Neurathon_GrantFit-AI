...
const GrantCard = ({
  ...
  return (
    <div className={`group bg-white border ${borderClass} rounded-2xl p-5 mb-4 transition-all duration-200`}>
      ...
      {warning && (
        <div className={`flex items-start gap-3 rounded-xl p-3 mb-4 border ${warning === 'High Alignment'
            ? 'bg-green-50 border-green-100 text-green-700'
            : warning === 'Potential Fit'
              ? 'bg-blue-50 border-blue-100 text-blue-700'
              : 'bg-amber-50 border-amber-100 text-amber-700'
          }`}>
          {warning === 'High Alignment' ? (
          ...
          <p className="text-[11px] font-bold italic leading-relaxed">
            {warning === 'High Alignment' ? 'AI Insight: ' : ''}
            {warning}
          </p>
        </div>
      )}
