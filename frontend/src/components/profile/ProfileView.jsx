...
const ProfileView = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f8] text-slate-900 font-['Public Sans',_sans-serif]">
      ...
      <div className="h-48 bg-[#1347ae] w-full relative overflow-hidden">
          ...
          <div className="lg:w-1/3 flex flex-col gap-6">
            <div className="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/60 border border-slate-100">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-[#1347ae] font-black text-sm uppercase tracking-widest mb-4">{profile.role}</p>
                ...
                <div className="w-full grid grid-cols-2 gap-4 py-6 border-y border-slate-100 mb-8">
                  ...
                  <div className="text-center border-l border-slate-100">
                    ...
                    <p className="text-sm font-bold text-[#1347ae]">Enterprise</p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <button className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-blue-50 text-[#1347ae] rounded-xl">
                      <Shield size={18} />
                    </div>
                    <span className="font-bold text-slate-700">Account Security</span>
                  </div>
                  <ChevronRight size={18} className="text-slate-300 group-hover:text-[#1347ae] transition-colors" />
                </button>
                <button className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-blue-50 text-[#1347ae] rounded-xl">
          ...
          <div className="flex-1 flex flex-col gap-8">
            <div className="bg-white rounded-3xl p-10 shadow-xl shadow-slate-200/40 border border-slate-100 flex-1">
              <div className="flex items-center justify-between mb-10">
                ...
                <Link
                  to="/dashboard"
                  className="flex items-center gap-2 text-slate-400 hover:text-[#1347ae] font-bold text-sm transition-colors"
                >
                  <ArrowLeft size={16} />
                  Dashboard
                </Link>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#1347ae] uppercase tracking-[0.2em] block">Domain / Industry</label>
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                    <Globe className="text-[#1347ae] size-5" />
                    <span className="font-bold text-slate-700">{profile.domain}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#1347ae] uppercase tracking-[0.2em] block">Funding Requirement</label>
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                    <DollarSign className="text-[#1347ae] size-5" />
                    <span className="font-bold text-slate-700">${Number(profile.fundingRequirement).toLocaleString()} USD</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#1347ae] uppercase tracking-[0.2em] block">Citizenship</label>
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                    <MapPin className="text-[#1347ae] size-5" />
                    <span className="font-bold text-slate-700">{profile.citizenship}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#1347ae] uppercase tracking-[0.2em] block">Primary Role</label>
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                    <Briefcase className="text-[#1347ae] size-5" />
                    <span className="font-bold text-slate-700">{profile.role}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#1347ae] uppercase tracking-[0.2em] block">Gender</label>
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                    <User className="text-[#1347ae] size-5" />
                    <span className="font-bold text-slate-700">{profile.gender}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#1347ae] uppercase tracking-[0.2em] block">Age</label>
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                    <Calendar className="text-[#1347ae] size-5" />
              ...
              {/* Data Privacy Note */}
              <div className="mt-16 p-6 bg-[#1347ae]/5 rounded-2xl border border-[#1347ae]/10 flex items-start gap-4">
                <Shield className="text-[#1347ae] size-6 mt-1" />
                <div>
                  <h4 className="font-bold text-[#1347ae] mb-1">Data Privacy Commitment</h4>
                  <p className="text-xs text-[#1347ae]/80 font-medium leading-relaxed">
